...
fn solve(data: &[u8]) -> u16 {
    let mut bean_split_count = 0;
    ...
    for (line_i, line) in data.chunks(line_size +2).enumerate() {
        if line_i % 2 == 0 {
            for ind in 0..line_size {
                ...
                else if c == b'^' && curr_line_buffer[ind] == 1{
                    new_line_buffer[ind-1] = 1;
                    new_line_buffer[ind+1] = 1;
                    if new_line_buffer[ind] != 1 {
                        new_line_buffer[ind] = 0;
                    }
                    bean_split_count += 1;
                }else if curr_line_buffer[ind] == 1{
                    new_line_buffer[ind] = 1;
                }
            }
            curr_line_buffer = new_line_buffer.clone();
            new_line_buffer = vec![0; line_size];
            // println!("{:?}", curr_line_buffer);
        }
    }
    bean_split_count
}